...
{
    [Preserve]
    public class TopicBroker<K, T> : ITopicBroker<K, T>, IDisposable
    {
        Options options;
        ...
        [Preserve]
        public TopicBroker(Options _options, HandlerFactory _handlerFactory, DiagnosticsInfo _diagnosticsInfo)
        {
            options = _options;
        ...
        public IDisposable Subscribe(K key, IBrokerHandler<T> handler, params BrokerHandlerFilter<T>[] filters)
        {
            lock (gate)
            {
                if (isDisposed)
                {
                    return options.HandlingSubscribeDisposedPolicy.Handle(nameof(TopicBroker<K, T>));
                }
        ...
        {
            List<IBrokerHandler<T>> handlers;
            TopicBroker<K, T> broker;

            public HandlerHolder(TopicBroker<K, T> _broker)
            {
