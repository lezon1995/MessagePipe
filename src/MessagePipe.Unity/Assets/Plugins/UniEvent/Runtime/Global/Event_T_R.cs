...
namespace UniEvent
{
    ...
    public static class Event<T, R>
    {
        ...
        {
            requester ??= Events.CreateEvent<T, R>();
        ...
        static Tuple<Func<T, UniTask<R>>, IDisposable> tuple2;
        static Dictionary<Func<T, UniTask<R>>, IDisposable> dict2;

        public static void SubTask(Func<T, UniTask<R>> handler)
        {
            requester ??= Events.CreateEvent<T, R>();
            var disposable = requester.Subscribe(handler);
            if (tuple2 == null)
            {
                if (dict2 == null)
                {
                    tuple2 = new Tuple<Func<T, UniTask<R>>, IDisposable>(handler, disposable);
                }
                else
                {
            ...
            else
            {
                if (dict2 == null)
                {
                    dict2 = new Dictionary<Func<T, UniTask<R>>, IDisposable>();
        ...
        public static void UnSubTask(Func<T, UniTask<R>> handler)
        {
