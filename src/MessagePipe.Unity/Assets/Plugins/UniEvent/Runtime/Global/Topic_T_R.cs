...
namespace UniEvent
{
    ...
    public static class Topic<T, R>
    {
        ...
        {
            requester ??= Events.CreateTopic<string, T, R>();
        ...
        public static void SubTask(string topic, Func<T, UniTask<R>> handler)
        {
            requester ??= Events.CreateTopic<string, T, R>();
            var disposable = requester.Subscribe(topic, handler);
        ...
        public static void UnSubTask(string topic, Func<T, UniTask<R>> handler)
        {
        ...
        {
            string topic;
            Func<T, UniTask<R>> handler;

            public Key2(string _topic, Func<T, UniTask<R>> _handler)
            {
                topic = _topic;
                handler = _handler;
            }
